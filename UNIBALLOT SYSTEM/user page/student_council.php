...
if (!empty($_SESSION['firstname']) && !empty($_SESSION['lastname'])) {
    ...
    $profile_sql = "SELECT firstname, lastname, department, program FROM students WHERE stu_no = ?";
    ...
    if ($profile_stmt) {
        ...
        if ($profile_result && $profile_result->num_rows > 0) {
            ...
            $student_department = $profile_row['department'] ?? '';
            $student_program = $profile_row['program'] ?? '';
            $initials = strtoupper(substr($firstname, 0, 1) . substr($lastname, 0, 1));
...
$student_department = $_SESSION['department'] ?? ($student_department ?? '');
...
if (!empty($student_department) && !empty($student_program)) {
    $sql = "SELECT * FROM candidates WHERE position IN ($placeholders) AND election_type = 'Student Council' AND department = ? AND program = ? ORDER BY position, lastname, firstname";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $bindParams = array_merge($positions, [$student_department, $student_program]);
        $types = str_repeat('s', count($bindParams));
    }
} elseif (!empty($student_department)) {
    $sql = "SELECT * FROM candidates WHERE position IN ($placeholders) AND election_type = 'Student Council' AND department = ? ORDER BY position, lastname, firstname";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $bindParams = array_merge($positions, [$student_department]);
        $types = str_repeat('s', count($bindParams));
    }
} else {
    $sql = "SELECT * FROM candidates WHERE position IN ($placeholders) AND election_type = 'Student Council' ORDER BY position, lastname, firstname";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $bindParams = $positions;
        $types = str_repeat('s', count($bindParams));
    }
}
