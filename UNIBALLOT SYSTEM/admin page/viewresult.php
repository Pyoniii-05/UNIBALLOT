...
foreach ($position_map as $position_title => $db_column) {
    ...
    if ($position_title === 'Student Council') {
        $sql = "SELECT c.id, c.firstname, c.lastname, c.position, c.department, c.program, c.party, c.photo, 
                       COUNT(v.stu_no) as vote_count 
                FROM candidates c 
                LEFT JOIN votes v ON v.$db_column = CONCAT(c.firstname, ' ', c.lastname) 
                WHERE c.election_type = 'Student Council' 
                GROUP BY c.id 
                            ...
                            <div class="candidate-header">
                                ...
                                <div>
                                    ...
                                    <?php if (!empty($candidate['position'])): ?>
                                        <div style="font-size:14px; color:#666; margin-top:4px;"><?php echo htmlspecialchars($candidate['position']); ?></div>
                                    <?php endif; ?>
                                    <div style="font-style:italic; font-size:14px; color:#444; margin-top:6px;"><?php echo htmlspecialchars($candidate['party']); ?></div>
                                </div>
                            </div>
                            <?php if (!empty($candidate['department']) || !empty($candidate['program'])): ?>
                                <div style="font-size:12px; color:#666; margin-top:10px;">
                                    <?php if (!empty($candidate['department'])): ?><?php echo htmlspecialchars($candidate['department']); ?><?php endif; ?>
                                    <?php if (!empty($candidate['department']) && !empty($candidate['program'])): ?> &middot; <?php endif; ?>
                                    <?php if (!empty($candidate['program'])): ?><?php echo htmlspecialchars($candidate['program']); ?><?php endif; ?>
                                </div>
                            <?php endif; ?>
