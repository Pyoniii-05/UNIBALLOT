...
if (isset($_POST['add_candidate'])) {
    ...
    $sql = "INSERT INTO candidates (firstname, lastname, position, party, year_level, department, program, message, photo) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssss", $firstname, $lastname, $position, $party, $year_level, $department, $program, $message, $photo_path);
...
if (isset($_POST['update_candidate'])) {
    ...
    $sql = "UPDATE candidates SET firstname=?, lastname=?, position=?, party=?, year_level=?, department=?, program=?, message=?, photo=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssssi", $firstname, $lastname, $position, $party, $year_level, $department, $program, $message, $photo_path, $id);
...
// Fetch candidates
$candidates = [];
$sql = "SELECT * FROM candidates ORDER BY position, firstname, lastname ASC";
                ...
                <table class="candidates-table" id="candidatesTable">
                    ...
                    <tbody id="candidatesTableBody">
                        <?php if (empty($candidates)): ?>
                            <tr><td colspan="6" style="text-align:center; padding:30px;">No candidates found.</td></tr>
                        <?php else: ?>
