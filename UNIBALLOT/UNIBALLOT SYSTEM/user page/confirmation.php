...
foreach ($positions_map as $key => $label) {
    ...
    if (!empty($val) && $val !== 'abstain') {
        if ($key === 'student_council') {
            $sql = "SELECT id, firstname, lastname, photo, party FROM candidates WHERE id = ? AND position = 'student_council' AND department = ?";
            $c_stmt = $conn->prepare($sql);
            $c_stmt->bind_param("is", $val, $_SESSION['department']);
        } else {
...
<body>
    ...
    <div class="drawer" id="drawer">
        ...
        <div class="drawer-nav">
            <?php if ($is_admin): ?>
                <a href="../admin page/dashboard.php" class="nav-item admin-link"><span class="material-icons">admin_panel_settings</span>Admin Dashboard</a>
            <?php endif; ?>
