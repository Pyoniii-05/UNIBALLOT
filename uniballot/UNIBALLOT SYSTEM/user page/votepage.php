...
foreach ($positions as $pos) {
    if ($pos === 'Student Council' && !empty($student_department)) {
        $c_sql = "SELECT * FROM candidates WHERE position = ? AND department = ? ORDER BY firstname, lastname";
        $c_stmt = $conn->prepare($c_sql);
        $c_stmt->bind_param("ss", $pos, $student_department);
    } else {
