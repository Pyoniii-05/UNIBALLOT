...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        #photo, #edit_photo, #add_photo { display: none; }
        ...
        @media (max-width: 768px) { 
            ...
            .search-and-filter-container { flex-direction: column; align-items: stretch; }
            .filter-select { width: 100%; margin-left: 0; }
            .section-title { flex-direction: column; align-items: flex-start; gap: 10px;}
            .btn-add-candidate { margin-left: 0; width: 100%; }
        }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <div class="content-card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 2px solid #e3e1c8;">
                ...
                <button class="btn-add-candidate" onclick="openAddModal()">
                    <span class="material-icons">person_add</span> Add Candidate
                </button>
            </div>
            ...
            <div class="search-and-filter-container">
                <div class="search-container">
                    ...
                    <input type="text" id="searchInput" class="form-control search-input" placeholder="Search by name, position, party, program, dept...">
                </div>
                    ...
                    <tbody id="candidatesTableBody">
                        ...
                        <?php else: ?>
                            <?php foreach ($candidates as $candidate): ?>
                                <tr data-election-type="<?php echo htmlspecialchars($candidate['election_type'] ?? 'USP'); ?>">
                                    <td>
                                    ...
                                    <td>
                                        <div style="font-size:12px; font-weight:700;"><?php echo htmlspecialchars($programFullNames[$candidate['program']] ?? $candidate['program']); ?></div>
    ...
    <div class="modal-overlay" id="addCandidateModal">
        <div class="modal-box large">
            ...
            <div style="padding: 0 30px 20px; overflow-y:auto; max-height: 70vh;">
                <form action="" method="POST" enctype="multipart/form-data" id="addCandidateForm">
                    <div class="form-row">
                        <div class="profile-upload-container">
                            <div class="profile-preview-wrapper">
                                <img id="add-modal-preview" class="preview-img" src="" alt="">
                                <span class="material-icons placeholder-icon" id="add-modal-placeholder">person</span>
                            </div>
                            <label for="add_photo" class="custom-file-upload">
                                <span class="material-icons" style="margin-right: 5px;">photo_camera</span> 
                                Upload Photo
                            </label>
                            <input type="file" id="add_photo" name="photo" accept="image/*" onchange="previewImage(this, 'add-modal-preview', 'add-modal-placeholder')">
                        </div>

                        <div style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                            
                            <div class="form-group">
                                <label for="add_election_type">Election Category</label>
                                <select id="add_election_type" name="election_type" class="form-control" required onchange="updatePositionOptions('add_election_type', 'add_position')">
                            ...
                            <div class="form-group">
                                ...
                                <select id="add_position" name="position" class="form-control" required>
                            ...
                            <div class="form-group">
                                <label for="add_party">Party/Affiliation</label>
                                <input type="text" id="add_party" name="party" class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label for="add_department">Department</label>
                                <select id="add_department" name="department" class="form-control" required onchange="updateAddProgramOptions()">
                                    <option value="" disabled selected>Select Department</option>
                                    <option value="CBA">College of Business Administration</option>
                                    <option value="CAS">College of Arts and Sciences</option>
                                    <option value="CCSE">College of Computer Studies and Engineering</option>
                            ...
                            <div class="form-group">
                                ...
                                <select name="program" id="add_program" class="form-control" required>
                                    <option value="" disabled selected>Select Program</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="add_year_level">Year Level</label>
                                <select id="add_year_level" name="year_level" class="form-control" required>
                            ...
                            <div class="form-group" style="grid-column: 1 / -1;">
                                <label for="add_message">Campaign Message</label>
                                <textarea id="add_message" name="message" class="form-control" rows="3" placeholder="Enter candidate's message..." required></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="modal-actions">
                        ...
                        <button type="submit" name="add_candidate" class="btn-modal confirm-success">Add Candidate</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <div class="form-row">
                        ...
                        <div style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                            <div class="form-group">
                                ...
                                <select id="edit_election_type" name="election_type" class="form-control" required onchange="updatePositionOptions('edit_election_type', 'edit_position')">
                                    <option value="USP">USP</option>
                                    <option value="Student Council">Student Council</option>
                                </select>
                            </div>
    ...
    <script>
        // Data Mapping
        const departmentPrograms = <?php echo json_encode($departmentPrograms); ?>;
        const programFullNames = <?php echo json_encode($programFullNames); ?>;

        const uspPositions = ["Prime Minister", "Executive Prime Minister", "Secretary General", "Treasurer", "Auditor"];
        const studentCouncilPositions = ["President", "Vice President", "Secretary", "Representative 1", "Representative 2", "Representative 3", "Representative 4"];

        function updatePositionOptions(electionTypeSelectId, positionSelectId, selectedPosition = null) {
            const electionTypeSelect = document.getElementById(electionTypeSelectId);
            ...
            if (selectedType === 'USP') {
                positionsToDisplay = uspPositions;
            } else if (selectedType === 'Student Council') {
                positionsToDisplay = studentCouncilPositions;
            }
            ...
            if (selectedPosition && positionsToDisplay.includes(selectedPosition)) {
                positionSelect.value = selectedPosition;
            }
        }

        function updateAddProgramOptions() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            updatePositionOptions('add_election_type', 'add_position');
            updateAddProgramOptions();
            ...
            if (tableBody && searchInput && filterSelect) {
                ...
                const filterAndSearchCandidates = () => {
                    const searchTerm = searchInput.value.toLowerCase().trim();
                    const selectedElectionType = filterSelect.value.toLowerCase();
                    ...
                    rows.forEach(row => {
                        ...
                        const party = row.children[4].textContent.toLowerCase();
                        const electionType = row.dataset.electionType ? row.dataset.electionType.toLowerCase() : '';
                        
                        // NEW: Capture Program and Department text from the 6th column
                        const programDept = row.children[5].textContent.toLowerCase();

                        // UPDATED: Added programDept search condition
                        const matchesSearch = name.includes(searchTerm) || 
                                              position.includes(searchTerm) || 
                                              party.includes(searchTerm) ||
                                              programDept.includes(searchTerm);
                        
                        const matchesFilter = selectedElectionType === 'all' || 
                ...
                searchInput.addEventListener('input', filterAndSearchCandidates);
                filterSelect.addEventListener('change', filterAndSearchCandidates);
            }
        ...
        function openAddModal() {
            ...
            form.reset(); 
            document.getElementById('add-modal-preview').src = "";
            document.getElementById('add-modal-preview').style.display = "none";
            document.getElementById('add-modal-placeholder').style.display = "block";
            updatePositionOptions('add_election_type', 'add_position');
            updateAddProgramOptions();
        ...
        function openEditModal(candidate) {
            ...
            updatePositionOptions('edit_election_type', 'edit_position', candidate.position);
